...
The plugin searches the XPath entry and compares it to a Condition value. It this is the right Condition value, the plugin changes the host and the path of the Route. 
...
### Example #5: replacing the values of XPath entries in ```calcWebService``` XML request
...
Use request defined at step #3, the expected result is no longer ```12``` but ```19```:
```xml
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" ...>
  <soap:Body>
    <AddResponse xmlns="http://tempuri.org/">
...
<faultstring>XSD validation failed: Element '{http://tempuri.org/}b': This element is not expected. Expected is ( {http://tempuri.org/}a ). (<string>, line 0)</faultstring>
```
Use request defined at step #3, **replace ```<b>7</b>``` by ```<b>ABCD</b>```** => there is an error because the ```<b>``` tag has the ```type="xs:integer"``` XSD property and Kong says: 
 ```xml
...
Use request defined at step #3, **remove ```<b>7</b>```** => there is an error because ```<b>``` is not present
...
```
Use request defined at step #3, **remove ```<b>7</b>```** => there is an error because ```<b>``` is present but with a String value instead of Integer
...
Use request defined at step #3, **remove ```<b>7</b>```** => there is no more error because: 
